...
describe("host commit-ish validation", () => {
  it("peels SHA, branch, and annotated tag to the same commit", async () => {
    ...
    for (const token of [sha, "main", "release"]) {
      assert.deepEqual(await resolveCommit(path, token), {
        status: "valid",
        commit: sha,
  ...
  it("separates invalid refs from operational repository failures", async () => {
    const path = repository();
    const canonical = realpathSync(path);
    ...
    for (const token of ["missing-ref", tree]) {
      ...
      assert.equal(result.repository, canonical);
    }
    assert.equal((await resolveCommit(join(path, "absent"), "main")).status, "operational_error");
  });
});
