...
function invalidCommitish(error: unknown): boolean {
  ...
  return /unknown revision|bad object|not a valid object name|needed a single revision|expected commit type|does not point to a commit/i.test(message);
}

/** Peel a requested ref to an exact commit without misclassifying host failures. */
export async function resolveCommit(
  ...
  try {
    repository = await realpath(await git(checkoutPath, "rev-parse", "--show-toplevel"));
  } catch (error) {
    return {
  ...
  try {
    const commit = await git(repository, "rev-parse", "--verify", "--quiet", `${requestedRef}^{commit}`);
    if (/^[0-9a-f]{40,64}$/.test(commit)) return { status: "valid", commit, repository };
    return { status: "operational_error", repository, reason: "git returned an unexpected commit identifier" };
  } catch (error) {
...
export default experimental_defineHostEntry({
  ...
  handlers: {
    ...
    async inspectWorktree({ checkoutPath }) {
      try {
        const status = await git(checkoutPath, "status", "--porcelain=v1", "--untracked-files=all");
        const lines = status.split("\n").map((line) => line.trim()).filter(Boolean);
        return {
          clean: lines.length === 0,
          summary: lines.length === 0 ? "clean" : lines.slice(0, 12).join("; ").slice(0, 1000),
        };
      } catch (error) {
        return { clean: false, summary: `could not inspect worktree: ${error instanceof Error ? error.message : String(error)}`.slice(0, 1000) };
      }
    },
